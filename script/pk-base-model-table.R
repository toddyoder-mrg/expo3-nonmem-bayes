...
thisScript <- "pk-base-model-table.R"
options(
...
fixed <- param_df %>% 
  ...
  st_rename("Estimate" = "value", 
            "95\\% CrI" = "ci") %>% 
  st_files(output = "pk-param-base-fixed.tex") %>% 
...
fixed_mcmc <- param_df %>% 
  ...
  st_rename("$\\hat{R}$" = "rhat", 
            "ESS bulk" = "ess_bulk",
            "ESS tail" = "ess_tail") %>% 
  st_files(output = "pk-param-base-fixed-mcmc.tex") %>% 
  st_notes(footMCMC) %>% 
  st_noteconf(type = "minipage", width = 1) %>% 
  stable() %T>% 
  # st2report(ntex = 2) %T>%
...
random <- param_df %>% 
  ...
  st_rename("Estimate" = "value", 
            "95\\% CrI" = "ci",
            "Shrinkage (\\%)" = "shrinkage") %>% 
  st_files(output = "pk-param-base-random.tex") %>% 
...
random_mcmc <- param_df %>% 
  ...
  st_rename("$\\hat{R}$" = "rhat", 
            "ESS bulk" = "ess_bulk",
            "ESS tail" = "ess_tail") %>% 
  st_files(output = "pk-param-base-random-mcmc.tex") %>% 
  st_notes(footMCMC) %>% 
  st_noteconf(type = "minipage", width = 1) %>% 
  stable() %T>% 
  # st2report(ntex = 2) %T>%
