...
# Create `diag` vector based on parameter name, checking entries like
# "OMEGA[2,3]" to see whether the numbers match. Returns NA for THETA
# entries.
get_diag <- function(.col) {
  extracted <- str_extract_all(.col, "\\d+")
  # Safely attempt to convert the first and second number,
  # return NA if not possible (for non-OMEGA/SIGMA)
  num1 <- map_dbl(extracted, possibly(~as.numeric(.x[1]), NA_real_))
  num2 <- map_dbl(extracted, possibly(~as.numeric(.x[2]), NA_real_))
  # Determine if numbers match, with NA where extraction failed
  diag <- num1 == num2
  diag
}

# Create tibble with summary of posterior draws
get_ptable <- function(.draws) {
  ...
  # Table functions need `random_effect_sd`:
  # OMEGA and SIGMA elements in standard deviation/correlation format
  # Function for extracting vector of correlations for either OMEGA or SIGMA
  get_om_sig_cor <- function(df, .type = "OMEGA") {
    df2 <- df %>% 
      filter(str_starts(name, .type))
    # OMEGA or SIGMA as covariance matrix
    matr <- df2 %>% 
      filter(str_starts(name, .type)) %>% 
      pull(estimate) %>% 
      mrgsolve::as_bmat()
    # Convert to correlation matrix, then extract upper triangle as vector
    matr_cor <- matr %>% 
      cov2cor() %>% 
      .[upper.tri(., diag = TRUE)]
    # Replace 1s on diagonal with SD (sqrt of diagonal)
    matr_cor[df2$diag] <- pull(sqrt(df2[df2$diag, "estimate"]))
    matr_cor
  }
  
  get_random_effect_sd <- function(df) {
    # Default (for THETAs) is NA
    df$random_effect_sd <- NA_real_
    df[str_starts(df$name, "OMEGA"), "random_effect_sd"] <- 
      get_om_sig_cor(df, .type = "OMEGA")
    df[str_starts(df$name, "SIGMA"), "random_effect_sd"] <- 
      get_om_sig_cor(df, .type = "SIGMA")
    df
  }
  ptable <- get_random_effect_sd(ptable)
  ptable
}
